...
set LIST_TITLE=ZAPRET: SNI Extension Split
...
--filter-tcp=443 --hostlist="%LIST_PATH%" --dpi-desync=multisplit --dpi-desync-split-pos=2,sniext+1 --dpi-desync-split-seqovl=679 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --new ^
...
--filter-tcp=443,%GModeRange% --ipset="%CLOUDFLARE_IPSET_PATH%" --dpi-desync=multisplit --dpi-desync-split-pos=2,sniext+1 --dpi-desync-split-seqovl=679 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --new ^
